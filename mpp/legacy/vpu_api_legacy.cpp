...
static MPP_RET setup_mlvec_ref_cfg(MppCtx mpp_ctx, MppApi *mpi, MppEncCfg enc_cfg,
                                   EncParameter_t *cfg)
{
    MPP_RET ret = MPP_OK;
...
RK_S32 VpuApiLegacy::control(VpuCodecContext *ctx, VPU_API_CMD cmd, void *param)
{
    ...
    case VPU_API_ENC_SETCFG : {
        ...
        memcpy(&enc_param, param, sizeof(enc_param));
        mpp_log_f("VPU_API_ENC_SETCFG extra_mode %2x max_tid %d\n",
                  enc_param.extra_mode, enc_param.max_tid);
        mpp_assert(enc_param.extra_mode == 0);
    ...
    case VPU_API_ENC_SET_MAX_TID: {
        ...
        enc_param.max_tid = max_tid;
        setup_mlvec_ref_cfg(mpp_ctx, mpi, enc_cfg, &enc_param);
        RK_S32 ret = mpi->control(mpp_ctx, MPP_ENC_SET_CFG, enc_cfg);
        if (ret)
            mpp_err("setup enc config failed ret %d\n", ret);
